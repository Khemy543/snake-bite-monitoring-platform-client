<template>
    <div class=" container">
            <div class="row row justify-content-center align-items-center">
              <div class="col-md-6">
                  <div class=" card p-4">
                <form @submit.prevent="EditDistrict">
                    <div class="row">
                        <div class=" col-md-6">
                            <base-input
                                type="text"
...
<script>
    export default {
        ...
        methods : {
            EditDistrict(){
                this.loading = true; 
                this.$axios.$post(`admin/update/${this.$route.params.id}/district`,{...this.form, _method:"PUT"})
                .then(response => {
                    this.$notify({ type:'success', message : 'District updated successfully' });
                })
                .catch(error => {
        ...
        async asyncData({ $axios, route }){
            try {
                const [firstResponse, secondResponse] = await Promise.all([ 
                    $axios.get(`admin/show/${route.params.id}/district`),
                    $axios.get("admin/fetch/regions")
                ]);
                
                if(firstResponse && firstResponse.data && secondResponse && secondResponse.data){
                    console.log(firstResponse.data)
                    return { 
                        form : { ...firstResponse.data, region_id:firstResponse.data.region.id},
                        regions: secondResponse.data
                    }
                }
            } catch (error) {
                console.log(error)
